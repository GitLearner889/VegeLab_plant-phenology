# Function original from Alejandro Sotillo

### Inverse Distance Weighting
## ROLE: 
#This function estimates values for unknown spatial location from known data of determined locations and values by using the principle of the closer point are the more similar the values should be. The exact formula : the sum of values of all points weighted by the distance of the points to the point of interpolation, divided by the sum of the distance between all the points and the interpolation point.
# All about it on: https://en.wikipedia.org/wiki/Inverse_distance_weighting

## ARGUMENTS:
# empty_positions: data-frame of position of all unknown points: id, location
# data_positions: data-frame of position of all known points: id, location
# data_values: data-frame of values of all known points: id, value (e.g. temperature) 
# beta: power parameter that give the speed at which weight decline with distance (great beta = only close sites matters) ; usually between 1 and 4
